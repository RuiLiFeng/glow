...
# Simpler, new version
@add_arg_scope
def revnet2d_step(name, z, logdet, hps, reverse, randomize_noise=True, noise_inputs=None, layer_idx=None):
    with tf.variable_scope(name):
        ...
        with tf.variable_scope('noise'):
            if randomize_noise:
                noise = tf.random_normal(z.shape, dtype=z.dtype)
            else:
                noise = tf.cast(noise_inputs[layer_idx], z.dtype)
            noise_strength = tf.get_variable('noise_strength', shape=[], initializer=tf.initializers.zeros())
            z += noise * tf.cast(noise_strength, z.dtype)
